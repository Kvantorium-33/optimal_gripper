/* СТРОКИ МОГУТ СБИТЬСЯ
  ТЕКУЩИЕ  ПУБЛИЧНЫЕ   ФУНКЦИИ 
.........................................ВХОДНЫЕ ДАННЫЕ
  com_init -        (строка: 241)               -
  
  Dynamixel_init -  (строка: 297)               -

  pins_init -       (строка: 224)               -

  encoders_init -   (строка: 248)               -

  read_endstops -   (строка: 261)               -

  print_encoders -  (строка: 279)               -
    
  print_endstops -  (строка: 268)               -

  pump -            (строка: 484)         HOLD/TAKE/PUT   

  gripper -         (строка: 479)             ПОИЗЦИЯ

  rotate_gripper -  (строка: 474)         0-300 ГРАДУСОВ

  move_X -          (строка: 322)        move_down/stop/up, РАЗРЕШЕНИЕ ДВИГАТЬСЯ 1 ДИНАМИКСЕЛЮ (TRUE/FALSE), РАЗРЕШЕНИЕ ДВИГАТЬСЯ 2 ДИНАМИКСЕЛЮ (TRUE/FALSE) 
  
  move_Y -          (строка: 432)        move_down/stop/up

  move_Z -          (строка: 362)        move_down/stop/up, РАЗРЕШЕНИЕ ДВИГАТЬСЯ 1 ДИНАМИКСЕЛЮ (TRUE/FALSE), РАЗРЕШЕНИЕ ДВИГАТЬСЯ 2 ДИНАМИКСЕЛЮ (TRUE/FALSE), РАЗРЕШЕНИЕ ДВИГАТЬСЯ 3 ДИНАМИКСЕЛЮ (TRUE/FALSE), РАЗРЕШЕНИЕ ДВИГАТЬСЯ 4 ДИНАМИКСЕЛЮ (TRUE/FALSE) 
*/
...
bool go_home()
{
  ...
  while (endstop_read[X1_arcell] != true && endstop_read[X2_arcell]) // СДВИГАЕМ ОСЬ Х ПОКА ВСЕ КОНЦЕВИКИ НЕ БУДУТ НАЖАТЫ
  {
    read_endstops(); // ЧИТАЕМ КОНЦЕВИКИ
  ...
  return true; // ВОЗВРАЩАЕМ ЧТО ВСЕ СДЕЛАНО
}
